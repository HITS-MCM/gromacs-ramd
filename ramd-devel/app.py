...
def update_graph(value):
    ...
    fig = ps.make_subplots(rows=nb_dist, cols=1, shared_xaxes=True,vertical_spacing=0.009,horizontal_spacing=0.009)
    fig['layout']['margin'] = {'l': 30, 'r': 10, 'b': 50, 't': 25}

    for r in range(nb_dist):
        fig.append_trace({'x':df.time,'y':df.iloc[:, r+1],'type':'scatter'}, r+1, 1)
